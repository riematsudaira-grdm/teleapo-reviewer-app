...
var SP = [
  ...
  "さらに、GRDMの営業設計上のポイントとして次も見極めに反映する。",
  "- 債務認識: お互い時間を投資したことへの敬意・責任感があるか（GRDMで最重要視される観点）。次回アポを軽く扱う・こちらの都合を一切気にしない素振りは、債務認識の欠如としてグレー〜要注意寄りに扱う。",
  "- 本音で話せているか: 建前・社交辞令に終始せず、本音レベルのやり取りができているか。表面的な相槌ばかりなら本音が取れていないサインとして指摘する。",
  "- 違和感: 相手の説明にモヤっとする点・不自然さがあれば、見逃さず言語化する（後で案件が潰れる芽になりやすい）。",
  "- 第三者/決裁権: 決裁権者が本人か。奥様・親などキーマンの存在や力関係が会話に出ていれば拾い、出ていなければ確認電話で押さえるべき点として扱う。",
...
async function run(){
  ...
  try{
    var res = await fetch("/api/app", {
      ...
      body: JSON.stringify({ model: MODEL, max_tokens: 4000, system: SP, messages:[{ role:"user", content: input }] })
    });
